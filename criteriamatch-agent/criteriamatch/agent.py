...
def _is_inapplicable(c) -> bool:
    """A not_met whose own rationale says the criterion doesn't apply is NOT a real
    disqualifier (e.g. smoking-cessation counseling for a non-smoker). Treat as satisfied."""
    text = f"{c.rationale} {getattr(c, 'evidence', '') or ''}".lower()
    return any(s in text for s in (
        "not applicable", "n/a", "does not apply", "not a current smoker",
        "non-smoker", "nonsmoker", "non smoker", "inapplicable", "not required",
    ))
...
def derive_routing(criteria: List[CriterionResult]) -> tuple[Determination, Routing, float]:
    ...
    real_not_met = [
        c for c in criteria
        if c.status == CriterionStatus.not_met and not _is_inapplicable(c)
    ]

    if real_not_met:
        ids = ", ".join(c.criterion_id for c in real_not_met)
        return (
            Determination.escalate,
            Routing(
                branch=Branch.escalate,
                reason=f"Criteria not met ({ids}); request appears guideline-discordant. "
                "Referred to a clinician - not auto-denied.",
            ),
            overall_conf,
        )

    if any(s in (CriterionStatus.missing, CriterionStatus.uncertain) for s in (c.status for c in criteria)):
        return (
